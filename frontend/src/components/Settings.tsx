...
const Settings: React.FC<SettingsProps> = ({ objects }) => {
  ...
  const [galaxyFile, setGalaxyFile] = useState<File | null>(null);
  ...
  // --- STATE PRO FIREMNÍ ÚDAJE (NOVÉ) ---
  const { data: companyData } = useCompanySettings();
  const saveCompanyMutation = useSaveCompanySettings();
  
  const [compForm, setCompForm] = useState<CompanySettings>({
      id: 'global_settings',
      name: '', ico: '', dic: '', phone: '', email: '',
  ...
  // Synchronizace formuláře firmy s načtenými daty
  useEffect(() => {
      if (companyData) {
          // Zajistíme, že address objekt existuje, i když přijde prázdný
          setCompForm({
              ...companyData,
              address: companyData.address || { street: '', city: '', zipCode: '' }
          });
      }
  }, [companyData]);
  ...
  const handleSaveCompany = (e: React.FormEvent) => {
      e.preventDefault();
      saveCompanyMutation.mutate(compForm, {
          onSuccess: () => {
              setIsSaved(true);
  ...
  const handleXMLImport = () => {
      ...
      reader.onload = (event) => {
          ...
          try {
              ...
              if (xmlDoc.getElementsByTagName("parsererror").length > 0) throw new Error("Chyba při parsování XML.");
              ...
              if (zones.length === 0) {
                  alert("V souboru nebyly nalezeny žádné zóny.");
                  return;
              }
              ...
              const blob = new Blob([outputContent], { type: 'text/plain;charset=windows-1250' });
      ...
      reader.onerror = () => alert("Chyba při čtení souboru.");
  };

  if (isLoading) return <div className="p-10 text-center text-gray-500">Načítám nastavení...</div>;

  // Společný styl pro inputy
  const inputClass = "w-full px-4 py-3 rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-900 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-500 dark:text-white";
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 pb-20">
          ...
          <div className="p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-800 flex flex-col items-center">
              ...
              {!galaxyFile ? (
                  <label className="cursor-pointer px-6 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-white rounded-xl font-bold text-sm hover:border-fuchsia-400 transition-all flex items-center gap-2">
                      ...
                      <input type="file" accept=".xml" className="hidden" onChange={(e) => setGalaxyFile(e.target.files?.[0] || null)} />
                  </label>
              ) : (
                  <div className="flex flex-col gap-3 w-full max-w-sm">
                      ...
                      <div className="flex gap-2">
                          <button onClick={handleXMLImport} className="flex-1 px-4 py-2 bg-fuchsia-600 hover:bg-fuchsia-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-fuchsia-500/20 active:scale-95 transition-all flex items-center justify-center gap-2">
                              <Download className="w-4 h-4" /> Spustit Export
                          </button>
                          <button onClick={() => setGalaxyFile(null)} className="px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded-xl font-bold">
                              Zrušit
                          </button>
                      </div>
                  </div>
      ...
      {currentUser?.role === 'ADMIN' ? (
        <>
            {/* 1. NASTAVENÍ FIRMY (DODAVATEL) - NOVÉ */}
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 mt-6">
                <div className="flex items-center space-x-3 mb-6">
                    ...
                    <div>
                        <h3 className="text-xl font-bold text-gray-800 dark:text-white">Naše firma (Dodavatel)</h3>
                        <p className="text-sm text-gray-500 dark:text-slate-400">Údaje pro hlavičku servisních protokolů a revizí.</p>
                    </div>
                </div>
                ...
                <form onSubmit={handleSaveCompany} className="space-y-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Název firmy</label>
                            <input required className={inputClass} value={compForm.name} onChange={e => setCompForm({...compForm, name: e.target.value})} placeholder="EL-SIGNÁL s.r.o." />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Ulice a číslo</label>
                            <input required className={inputClass} value={compForm.address.street} onChange={e => setCompForm({...compForm, address: {...compForm.address, street: e.target.value}})} />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Město</label>
                            <input required className={inputClass} value={compForm.address.city} onChange={e => setCompForm({...compForm, address: {...compForm.address, city: e.target.value}})} />
                        </div>
                        <div>
                            <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">PSČ</label>
                            <input required className={inputClass} value={compForm.address.zipCode} onChange={e => setCompForm({...compForm, address: {...compForm.address, zipCode: e.target.value}})} />
                        </div>
                        
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">IČ</label>
                                <input required className={inputClass} value={compForm.ico} onChange={e => setCompForm({...compForm, ico: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">DIČ</label>
                                <input className={inputClass} value={compForm.dic} onChange={e => setCompForm({...compForm, dic: e.target.value})} />
                            </div>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input className={inputClass} value={compForm.phone} onChange={e => setCompForm({...compForm, phone: e.target.value})} />
                            </div>
                            <div>
                                <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">E-mail</label>
                                <input className={inputClass} value={compForm.email} onChange={e => setCompForm({...compForm, email: e.target.value})} />
                            </div>
                        </div>
                    </div>
                    <div className="flex justify-end pt-2">
                        <button type="submit" disabled={saveCompanyMutation.isPending} className="px-6 py-3 bg-blue-600 text-white rounded-xl font-bold shadow-lg hover:bg-blue-700 transition flex items-center gap-2">
                            {saveCompanyMutation.isPending ? <Loader2 className="animate-spin w-4 h-4"/> : <Save className="w-4 h-4" />}
                            Uložit údaje
                        </button>
                    </div>
                </form>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 mt-6">
                ...
                <form onSubmit={handleCreateBt} className="grid grid-cols-1 md:grid-cols-12 gap-4 mb-6 p-4 bg-gray-50 dark:bg-slate-800/50 rounded-2xl border border-gray-100 dark:border-slate-800">
                    <div className="md:col-span-4">
                        ...
                        <input className={inputClass} required value={newBt.name} onChange={e => setNewBt({...newBt, name: e.target.value})} placeholder="NPL 17-12" />
                    </div>
                    <div className="md:col-span-3">
                        <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Výrobce</label>
                        <input className={inputClass} value={newBt.manufacturer} onChange={e => setNewBt({...newBt, manufacturer: e.target.value})} placeholder="Yuasa" />
                    </div>
                    <div className="md:col-span-2">
                         <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Kapacita (Ah)</label>
                        <input className={inputClass} type="number" step="0.1" required value={newBt.capacityAh} onChange={e => setNewBt({...newBt, capacityAh: e.target.value})} placeholder="17" />
                    </div>
                    <div className="md:col-span-2">
                        <label className="text-[10px] font-bold text-gray-400 uppercase ml-1">Napětí (V)</label>
                        <input className={inputClass} type="number" step="0.1" required value={newBt.voltageV} onChange={e => setNewBt({...newBt, voltageV: e.target.value})} placeholder="12" />
                    </div>
                    <div className="md:col-span-1 flex items-end">
                        <button type="submit" className="w-full py-3 bg-green-600 text-white rounded-xl font-bold shadow-lg shadow-green-500/20 hover:bg-green-700 active:scale-95 transition flex items-center justify-center">
                    ...
                    {batteryTypes.map(bt => (
                        <div key={bt.id} className="flex items-center justify-between p-3 border border-gray-100 dark:border-slate-800 rounded-xl hover:bg-gray-50 dark:hover:bg-slate-800/50 transition bg-white dark:bg-slate-900">
                            <div className="flex items-center gap-3">
                                ...
                                <div>
                                    <div className="font-bold text-gray-800 dark:text-white text-sm">{bt.manufacturer} {bt.name}</div>
                                    <div className="text-xs text-gray-400">Kapacita: <span className="font-bold text-gray-600 dark:text-slate-300">{bt.capacityAh} Ah</span></div>
                                </div>
                            </div>
                            <button onClick={() => { if(confirm('Smazat tento typ?')) deleteBatteryTypeMutation.mutate(bt.id) }} className="text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 p-2 rounded-lg transition">
                                <Trash2 className="w-4 h-4" />
                            </button>
                        </div>
                    ))}
                    {batteryTypes.length === 0 && <div className="text-center py-4 text-gray-400 text-sm italic border-2 border-dashed border-gray-100 dark:border-slate-800 rounded-xl">Žádné typy baterií.</div>}
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 mt-6">
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center space-x-3">
                        <div className="p-3 bg-indigo-50 dark:bg-indigo-500/10 rounded-2xl text-indigo-600 dark:text-indigo-400"><ClipboardList className="w-6 h-6" /></div>
                        <div>
                ...
                <div className="space-y-4">
                    ...
                    <div key={tpl.id} className="flex items-center justify-between p-5 border border-gray-100 dark:border-slate-800 rounded-2xl hover:bg-gray-50 dark:hover:bg-slate-800/50 transition group">
                        <div className="flex items-center space-x-4">
                            <div className="p-2 bg-gray-100 dark:bg-slate-800 text-gray-500 dark:text-slate-400 rounded-xl"><Settings2 className="w-5 h-5" /></div>
                            <div>
                                <p className="font-bold text-gray-800 dark:text-white text-lg">{tpl.name}</p>
                                <p className="text-xs font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest">{tpl.fields.length} polí</p>
                            </div>
                        </div>
                        <button onClick={() => setEditingTemplate(tpl)} className="px-4 py-2 bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-700 text-gray-600 dark:text-slate-300 rounded-xl font-bold text-sm shadow-sm hover:ring-2 hover:ring-blue-500/20 transition-all">Upravit</button>
                    </div>
                    ))}
                </div>
            </div>
            ...
            <div className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] shadow-sm border border-gray-100 dark:border-slate-800 mt-6">
                <div className="flex items-center space-x-3 mb-6">
                    <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 rounded-2xl text-emerald-600 dark:text-emerald-400"><Database className="w-6 h-6" /></div>
                    <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-xs text-gray-400 mb-4 max-w-[200px]">Stáhne ZIP archiv obsahující všechna data a nahrané soubory.</p>
                        <button onClick={handleDownloadBackup} className="px-6 py-3 bg-blue-600 hover:bg-blue-700 text-white rounded-xl font-bold text-sm shadow-lg shadow-blue-500/20 active:scale-95 transition-all flex items-center gap-2">
                    ...
                    <div className="p-6 bg-slate-50 dark:bg-slate-800/50 rounded-2xl border border-dashed border-gray-200 dark:border-slate-700 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-xs text-gray-400 mb-4 max-w-[200px]">Nahrajte ZIP soubor. <span className="text-red-500 font-bold">Pozor: Data budou přepsána!</span></p>
                        {!restoreFile ? (
                            <label className="cursor-pointer px-6 py-3 bg-white dark:bg-slate-700 border border-gray-200 dark:border-slate-600 text-gray-700 dark:text-white rounded-xl font-bold text-sm hover:border-blue-400 transition-all flex items-center gap-2">
                                ...
                                <input type="file" accept=".zip" className="hidden" onChange={(e) => setRestoreFile(e.target.files?.[0] || null)} />
                            </label>
                        ) : (
                            <div className="flex flex-col gap-3 w-full">
                                <div className="flex items-center justify-center gap-2 text-sm font-bold text-gray-700 dark:text-slate-300 bg-white dark:bg-slate-900 p-2 rounded-lg">
                                    <CheckCircle className="w-4 h-4 text-green-500" /> {restoreFile.name}
                                </div>
                                <div className="flex gap-2">
                                    <button onClick={handleRestoreBackup} disabled={isRestoring} className="flex-1 px-4 py-2 bg-amber-500 hover:bg-amber-600 text-white rounded-xl font-bold text-sm shadow-lg shadow-amber-500/20 active:scale-95 transition-all flex items-center justify-center gap-2">
                                        {isRestoring ? <RefreshCw className="w-4 h-4 animate-spin" /> : <AlertTriangle className="w-4 h-4" />}
                                        {isRestoring ? 'Obnovuji...' : 'Spustit obnovu'}
                                    </button>
                                    <button onClick={() => setRestoreFile(null)} disabled={isRestoring} className="px-4 py-2 bg-gray-200 dark:bg-slate-700 text-gray-600 dark:text-slate-300 rounded-xl font-bold">Zrušit</button>
                                </div>
                            </div>
                        )}
                    </div>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh]">
            <div className="px-8 py-6 bg-slate-50 dark:bg-slate-800/50 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center">
              <div>
                ...
                <input className="bg-transparent font-black text-2xl text-blue-600 focus:outline-none placeholder-blue-300 w-full" value={editingTemplate.name} onChange={(e) => setEditingTemplate({...editingTemplate, name: e.target.value})} />
              </div>
              <button onClick={() => setEditingTemplate(null)} className="p-2 hover:bg-gray-200 dark:hover:bg-slate-700 rounded-full transition text-slate-400"><X className="w-6 h-6" /></button>
            </div>
            <div className="p-8 overflow-y-auto flex-1 space-y-6 bg-white dark:bg-slate-900">
              <div className="flex justify-between items-center">
                ...
                <button onClick={addFieldToTemplate} className="text-xs bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 font-bold px-3 py-1.5 rounded-lg flex items-center hover:bg-blue-100 transition"><Plus className="w-4 h-4 mr-1" /> Přidat pole</button>
              </div>
              <div className="space-y-4">
                    ...
                    <div className="flex-1 space-y-3">
                      <div className="flex gap-4">
                        <div className="flex-1">
                          ...
                          <input className={inputClass.replace('py-3', 'py-2')} value={field.label} onChange={(e) => updateField(field.id, { label: e.target.value })} placeholder="Např. Teplota, Stav..." />
                        </div>
                        <div className="w-1/3">
                          <label className="block text-[10px] font-bold uppercase text-gray-400 mb-1">Typ</label>
                          <select className={inputClass.replace('py-3', 'py-2')} value={field.type} onChange={(e) => updateField(field.id, { type: e.target.value as any })}>
                      ...
                      {field.type === 'select' && (
                        <div>
                           ...
                           <input className={inputClass.replace('py-3', 'py-2')} value={field.options?.join(', ') || ''} onChange={(e) => updateField(field.id, { options: e.target.value.split(',').map(s => s.trim()) })} placeholder="Možnost A, Možnost B, ..." />
                        </div>
                      )}
                      <div className="flex items-center">
                        <label className="flex items-center text-xs font-bold text-gray-600 dark:text-slate-400 cursor-pointer select-none">
                          <input type="checkbox" className="mr-2 w-4 h-4 rounded text-blue-600 focus:ring-blue-500" checked={field.required} onChange={(e) => updateField(field.id, { required: e.target.checked })} /> 
                          Povinné pole
                        </label>
                      </div>
                    </div>
                    <button onClick={() => removeField(field.id)} className="self-center p-3 text-gray-300 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-xl transition" title="Odstranit pole"><Trash2 className="w-5 h-5" /></button>
                  </div>
                ))}
              </div>
            </div>
            <div className="p-6 bg-slate-50 dark:bg-slate-800/50 border-t border-gray-100 dark:border-slate-800 flex gap-4">
              <button onClick={() => setEditingTemplate(null)} className="flex-1 py-4 border-2 border-gray-200 dark:border-slate-700 rounded-2xl text-gray-600 dark:text-slate-300 font-bold hover:bg-white dark:hover:bg-slate-800 transition">Zrušit</button>
              <button onClick={() => { const updatedTemplates = templates.map(t => t.id === editingTemplate.id ? editingTemplate : t); saveTemplates(updatedTemplates); }} className="flex-1 py-4 bg-blue-600 text-white rounded-2xl font-black shadow-xl shadow-blue-500/20 hover:bg-blue-700 active:scale-95 transition">Uložit změny</button>
            </div>
          </div>
        </div>
      )}
