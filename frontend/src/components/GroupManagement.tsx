...
const GroupManagement: React.FC<GroupManagementProps> = ({ groups, objects }) => {
  ...
  const [formData, setFormData] = useState<Partial<ObjectGroup>>({
    ...
    billingInfo: { name: '', ico: '', dic: '', address: { street: '', city: '', zipCode: '' }, isVatPayer: true }
  });
  ...
  const startEditing = (group: ObjectGroup) => {
    ...
    setFormData({
      ...
      // NOVÉ: Načtení nebo default
      billingInfo: group.billingInfo || {
        name: group.name, // Předvyplníme název skupiny
        ico: '', dic: '',
  ...
  return (
    ...
    <h4 className="text-sm font-black text-gray-500 dark:text-slate-400 uppercase tracking-widest mb-4">
        Fakturační údaje (Objednatel)
    </h4>
    
    <div className="space-y-4">
        <div>
            <label className="label-tiny">Obchodní název</label>
            <input 
                className="input-std" 
                value={formData.billingInfo?.name} 
                onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, name: e.target.value}})}
                placeholder="ČSOB a.s."
            />
        </div>
        
        <div className="grid grid-cols-2 gap-4">
            <div>
                <label className="label-tiny">IČ</label>
                <input 
                    className="input-std" 
                    value={formData.billingInfo?.ico} 
                    onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, ico: e.target.value}})}
                />
            </div>
            <div>
                <label className="label-tiny">DIČ</label>
                <input 
                    className="input-std" 
                    value={formData.billingInfo?.dic} 
                    onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, dic: e.target.value}})}
                />
            </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
            <div className="col-span-3">
                <label className="label-tiny">Ulice</label>
                <input 
                    className="input-std" 
                    value={formData.billingInfo?.address.street} 
                    onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, address: {...formData.billingInfo!.address, street: e.target.value}}})}
                    placeholder="Radlická 333/150"
                />
            </div>
            <div>
                <label className="label-tiny">Město</label>
                <input 
                    className="input-std" 
                    value={formData.billingInfo?.address.city} 
                    onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, address: {...formData.billingInfo!.address, city: e.target.value}}})}
                    placeholder="Praha 5"
                />
            </div>
            <div>
                <label className="label-tiny">PSČ</label>
                <input 
                    className="input-std" 
                    value={formData.billingInfo?.address.zipCode} 
                    onChange={e => setFormData({...formData, billingInfo: {...formData.billingInfo!, address: {...formData.billingInfo!.address, zipCode: e.target.value}}})}
                    placeholder="150 57"
                />
            </div>
        </div>
