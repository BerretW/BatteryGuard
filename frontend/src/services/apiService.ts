...
export interface IApiService {
  ...
  saveGroups(groups: ObjectGroup[]): Promise<void>; // Bulk save
  // --- NOVÉ METODY PRO SKUPINY ---
  createGroup(group: Partial<ObjectGroup>): Promise<void>;
  updateGroup(groupId: string, updates: Partial<ObjectGroup>): Promise<void>;
...
async getCompanySettings(): Promise<CompanySettings> { return this.request('/settings/company'); }
  ...
  async updateTechnology(objId: string, techId: string, updates: any): Promise<void> { return this.request(`/objects/${objId}/technologies/${techId}`, 'PATCH', updates); }
  ...
  async updateBatteryStatus(objId: string, techId: string, batId: string, status: BatteryStatus, extraData: any = {}): Promise<void> { return this.request(`/objects/${objId}/technologies/${techId}/batteries/${batId}`, 'PATCH', { status, ...extraData }); }
  ...
  async createGroup(group: Partial<ObjectGroup>): Promise<void> { return this.request('/groups', 'POST', group); }
  async updateGroup(groupId: string, updates: Partial<ObjectGroup>): Promise<void> { return this.request(`/groups/${groupId}`, 'PATCH', updates); }
  async deleteGroup(groupId: string): Promise<void> { return this.request(`/groups/${groupId}`, 'DELETE'); }
  // ---------------------------------
  ...
  async updateUserPassword(userId: string, newPassword: string): Promise<void> { return this.request(`/users/${userId}/password`, 'PATCH', { newPassword }); }
  async updateSelfPassword(currentPassword: string, newPassword: string): Promise<void> { return this.request('/auth/password', 'PATCH', { currentPassword, newPassword }); }
}
